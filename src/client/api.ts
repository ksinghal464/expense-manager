import type {
  Account, Category, PaymentMethod, Payee, Tag, TxView, Bootstrap, Dashboard,
  ImportSummary, RecurringRule, Note, AuditEntry,
} from '../shared/types';

export type SplitRow = { id: string; category_id: string | null; category_name?: string | null; amount_minor: number; description: string; note: string };
export type AttachmentRow = { id: string; transaction_id: string; provider: string; external_file_id: string; file_name: string; mime_type: string; size_bytes: number | null; kind: string; url: string };
export type TxDetail = TxView & { splits: SplitRow[]; refunds: TxView[]; tags: string[]; refunded_minor: number };
...
const post = <T = unknown>(path: string, body: unknown): Promise<T> => req<T>(path, { method: 'POST', headers: J, body: JSON.stringify(body) });
const put = <T = unknown>(path: string, body: unknown): Promise<T> => req<T>(path, { method: 'PUT', headers: J, body: JSON.stringify(body) });

export const api = {
  health: () => req<{ ok: boolean }>('/api/health'),
  bootstrap: () => req<Bootstrap>('/api/bootstrap'),
  dashboard: () => req<Dashboard>('/api/dashboard'),
  searchOptions: (q: string) => req<SearchOptions>(`/api/search-options?q=${encodeURIComponent(q)}`),
  ...
  createTransaction: (body: Record<string, unknown>) => req<TxDetail>('/api/transactions', { method: 'POST', headers: J, body: JSON.stringify(body) }),
  updateTransaction: (id: string, body: Record<string, unknown>) => req<TxDetail>(`/api/transactions/${id}`, { method: 'PUT', headers: J, body: JSON.stringify(body) }),
  deleteTransaction: (id: string, hard = false) => req<{ ok: boolean }>(`/api/transactions/${id}${hard ? '?hard=1' : ''}`, { method: 'DELETE' }),
  ...
  saveAccount: (id: string | null, body: Record<string, unknown>) => (id ? put(`/api/accounts/${id}`, body) : post('/api/accounts', body)),
  deleteAccount: (id: string) => req(`/api/accounts/${id}`, { method: 'DELETE' }),

  categories: () => req<Category[]>('/api/categories'),
  saveCategory: (id: string | null, body: Record<string, unknown>) => (id ? put(`/api/categories/${id}`, body) : post('/api/categories', body)),
  deleteCategory: (id: string) => req(`/api/categories/${id}`, { method: 'DELETE' }),

  paymentMethods: () => req<PaymentMethod[]>('/api/payment-methods'),
  savePaymentMethod: (id: string | null, body: Record<string, unknown>) => (id ? put(`/api/payment-methods/${id}`, body) : post('/api/payment-methods', body)),
  deletePaymentMethod: (id: string) => req(`/api/payment-methods/${id}`, { method: 'DELETE' }),

  payees: () => req<Payee[]>('/api/payees'),
  savePayee: (id: string | null, body: Record<string, unknown>) => (id ? put(`/api/payees/${id}`, body) : post('/api/payees', body)),
  deletePayee: (id: string) => req(`/api/payees/${id}`, { method: 'DELETE' }),

  tags: () => req<Tag[]>('/api/tags'),
  saveTag: (id: string | null, body: Record<string, unknown>) => (id ? put(`/api/tags/${id}`, body) : post('/api/tags', body)),
  ...
  saveNote: (id: string | null, body: Record<string, unknown>) => (id ? put(`/api/notes/${id}`, body) : post('/api/notes', body)),
  ...
  saveRecurring: (id: string | null, body: Record<string, unknown>) => (id ? put(`/api/recurring/${id}`, body) : post('/api/recurring', body)),
  ...
  toggleRecurring: (id: string) => req<RecurringRule>(`/api/recurring/${id}/toggle`, { method: 'PATCH' }),
  ...
  exportCsv: (qs?: Record<string, string>) => {
    ...
    return fetch(`/api/export/csv${p}`).then((r) => {
      if (!r.ok) return r.text().then((t) => { throw new ApiError(r.status, t); });
      return r.text();
    });
  },
  exportJson: () => req<unknown>('/api/export/json'),
  restoreBackup: (backup: unknown) => post<{ restored: number }>('/api/import/backup', backup),
  driveStatus: () => req<{ configured: boolean }>('/api/drive/status'),
  driveSync: () => post<{ url?: string }>('/api/drive/sync', {}),
