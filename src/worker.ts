...
async function audit(env: Env, entityType: string, entityId: string, action: string, before: unknown, after: unknown) {
  await env.DB.prepare(`INSERT INTO audit_log (id, occurred_at, entity_type, entity_id, action, before_json, after_json) VALUES (?, ?, ?, ?, ?, ?, ?)`)
    .bind(id(), now(), entityType, entityId, action, before == null ? null : JSON.stringify(before), after == null ? null : JSON.stringify(after)).run();
}
  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    try {
      ...
      if (request.method === 'GET' && url.pathname === '/api/dashboard') {
        ...
        const transactions = rows.results; const today=new Date();
        const startOfWeek=new Date(today); startOfWeek.setHours(0,0,0,0); startOfWeek.setDate(today.getDate()-((today.getDay()+6)%7));
      ...
      if (request.method === 'POST' && url.pathname === '/api/categories') {
        const b=await body(request),name=String(b.name||'').trim(),kind=String(b.kind||'expense'); if(!name||!['expense','income','both'].includes(kind))return json({error:'Invalid category'},{status:400});
        ...
        await env.DB.prepare(`INSERT INTO categories (id,name,parent_id,kind,sort_order,is_active,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?)`).bind(category.id,category.name,category.parent_id,category.kind,0,1,created,created).run();
      ...
      if (request.method === 'POST' && url.pathname === '/api/transactions') {
        ...
        const created=now(),occurredAt=String(b.occurredAt||created),description=String(b.description||'').trim(),note=String(b.note||''),categoryId=b.categoryId?String(b.categoryId):null,paymentMethodId=b.paymentMethodId?String(b.paymentMethodId):null,status=b.status==='uncleared'?'uncleared':'cleared';
