...
async function upsertPayee(env: Env, name: string, at: string): Promise<string | null> {
  const n = name.trim();
  if (!n) return null;
  const r = await env.DB.prepare('SELECT id FROM payees WHERE lower(name)=? AND deleted_at IS NULL LIMIT 1')
    .bind(n.toLowerCase())
    .first<Row>();
  if (r) return r.id;
  const newId = id();
...
async function upsertTag(env: Env, name: string, at: string): Promise<string | null> {
  const n = name.trim();
  if (!n) return null;
  const r = await env.DB.prepare('SELECT id FROM tags WHERE lower(name)=? AND deleted_at IS NULL LIMIT 1')
    .bind(n.toLowerCase())
    .first<Row>();
  if (r) return r.id;
  const newId = id();
...
async function txBase(env: Env, txId: string): Promise<Row | null> {
  return env.DB.prepare(`${TX_SELECT} WHERE t.id=?`)
    .bind(txId)
    .first<Row>();
}
...
async function txDetail(env: Env, txId: string): Promise<Row | null> {
  ...
  return { ...t, splits: splits.results, refunds: refunds.results, tags, refunded_minor: refunded?.s ?? 0 };
}
...
async function listTransactions(env: Env, url: URL): Promise<Response> {
  ...
  const r = await env.DB.prepare(
    ...
    .all<Row>();
  const tags = await tagsForMany(env, r.results.map((x) => x.id));
...
async function createTransaction(env: Env, request: Request): Promise<Response> {
  ...
  const acc = await env.DB.prepare('SELECT 1 AS ok FROM accounts WHERE id=? AND deleted_at IS NULL').bind(accountId).first<Row>();
  if (!acc) throw new HttpError(400, 'Account not found');

  const categoryId = optStr(b, 'categoryId');
  const methodId = optStr(b, 'methodId');
  if (categoryId && !(await exists(env, 'categories', categoryId))) throw new HttpError(400, 'Category not found');
  if (methodId && !(await exists(env, 'payment_methods', methodId))) throw new HttpError(400, 'Payment method not found');

  const refundsTransactionId = optStr(b, 'refundsTransactionId');
  if (refundsTransactionId) {
    const ref = await env.DB.prepare('SELECT transaction_type FROM transactions WHERE id=? AND deleted_at IS NULL')
      .bind(refundsTransactionId)
      .first<Row>();
    if (!ref) throw new HttpError(400, 'Refund target transaction not found');
    if (ref.transaction_type !== 'expense') throw new HttpError(400, 'Refund target must be an expense');
  ...
  const quantity = qRaw === undefined || qRaw === null || String(qRaw).trim() === '' ? null : Number(qRaw);
  ...
  if (isSplitParent) {
    ...
    const prepared = splitsRaw.map((s) => {
      ...
      return { id: id(), categoryId: catId, amount: amt, description: str(s, 'description'), note: str(s, 'note') };
    });
    for (const s of prepared) if (s.categoryId && !(await exists(env, 'categories', s.categoryId))) throw new HttpError(400, 'Split category not found');
    if (Math.abs(sum - amountMinor) > 1) throw new HttpError(400, 'Split amounts must sum to the total');
    await env.DB.prepare(INSERT_TX)
      .bind(txId, accountId, methodId, categoryId, payeeId, type, amountMinor, occurredAt, description, note, status, referenceNumber, taxMinor, quantity, unit, refundsTransactionId, null, null, 1, at, at)
      .run();
    await env.DB.batch(prepared.map((s) => env.DB.prepare(
    ...
    await env.DB.prepare(INSERT_TX)
      .bind(txId, accountId, methodId, categoryId, payeeId, type, amountMinor, occurredAt, description, note, status, referenceNumber, taxMinor, quantity, unit, refundsTransactionId, null, null, 0, at, at)
      .run();
  }

  const tagNames = Array.isArray(b['tags']) ? (b['tags'] as any[]).map((t) => String(t).trim()).filter(Boolean) : [];
  const tagIds: string[] = [];
  ...
  if (tagIds.length) {
    await env.DB.batch(tagIds.map((tid) => env.DB.prepare('INSERT OR IGNORE INTO transaction_tags (transaction_id,tag_id) VALUES (?,?)').bind(txId, tid)));
  }

  if (description) {
    const ex = await env.DB.prepare('SELECT id FROM description_suggestions WHERE lower(description)=lower(?)').bind(description).first<Row>();
    if (ex) await env.DB.prepare('UPDATE description_suggestions SET usage_count=usage_count+1, last_used_at=?, updated_at=? WHERE id=?').bind(at, at, ex.id).run();
    else await env.DB.prepare('INSERT INTO description_suggestions (id,description,usage_count,last_used_at,created_at,updated_at) VALUES (?,?,1,?,?,?)').bind(id(), description, at, at, at).run();
  }
...
async function updateTransaction(env: Env, request: Request, txId: string): Promise<Response> {
  const before = await env.DB.prepare('SELECT * FROM transactions WHERE id=? AND deleted_at IS NULL').bind(txId).first<Row>();
  ...
  const methodId = b['methodId'] === null ? null : b['methodId'] !== undefined ? String(b['methodId']) : before.payment_method_id;
  const categoryId = b['categoryId'] === null ? null : b['categoryId'] !== undefined ? String(b['categoryId']) : before.category_id;
  if (methodId && !(await exists(env, 'payment_methods', methodId))) throw new HttpError(400, 'Payment method not found');
  if (categoryId && !(await exists(env, 'categories', categoryId))) throw new HttpError(400, 'Category not found');

  let payeeId = b['payeeId'] === null ? null : b['payeeId'] !== undefined ? String(b['payeeId']) : before.payee_id;
  if (b['payee'] !== undefined) payeeId = await upsertPayee(env, str(b, 'payee'), at);

  const status = b['status'] !== undefined ? (STATUS_RE.test(String(b['status'])) ? String(b['status']) : 'cleared') : before.status;
  const description = b['description'] !== undefined ? String(b['description']) : before.description;
  const note = b['note'] !== undefined ? String(b['note']) : before.note;
  const referenceNumber = b['referenceNumber'] !== undefined ? String(b['referenceNumber']) : before.reference_number || '';
  const taxNum = b['tax'] !== undefined ? Number(b['tax']) : (before.tax_minor || 0) / 100;
  const taxMinor = Number.isFinite(taxNum) ? Math.round(taxNum * 100) : 0;
  const quantity = b['quantity'] !== undefined && b['quantity'] !== null && String(b['quantity']).trim() !== '' ? Number(b['quantity']) : before.quantity ?? null;
  ...
  await env.DB.prepare(
    ...
    .bind(accountId, methodId, categoryId, payeeId, type, amountMinor, occurredAt, description, note, status, referenceNumber, taxMinor, quantity, unit, at, txId)
    .run();
  ...
  if (Array.isArray(b['splits'])) {
    ...
    if (splitsRaw.length >= 2) {
      ...
      const prepared = splitsRaw.map((s) => {
        ...
        if (!(amt > 0)) throw new HttpError(400, 'Each split amount must be greater than zero');
        sum += amt;
        return { id: id(), categoryId: optStr(s, 'categoryId'), amount: amt, description: str(s, 'description'), note: str(s, 'note') };
      });
      if (Math.abs(sum - amountMinor) > 1) throw new HttpError(400, 'Split amounts must sum to the total');
      await env.DB.batch(prepared.map((s) => env.DB.prepare(
        'INSERT INTO transaction_splits (id,transaction_id,category_id,amount_minor,description,note,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?)',
    ...
    await env.DB.prepare('UPDATE transactions SET is_split_parent=? WHERE id=?').bind(isSplitParent, txId).run();
  }
  ...
  if (Array.isArray(b['tags'])) {
    ...
    if (tagIds.length) await env.DB.batch(tagIds.map((tid) => env.DB.prepare('INSERT OR IGNORE INTO transaction_tags (transaction_id,tag_id) VALUES (?,?)').bind(txId, tid)));
  }
  ...
  const after = await env.DB.prepare('SELECT * FROM transactions WHERE id=?').bind(txId).first<Row>();
...
async function deleteTransaction(env: Env, txId: string, hard: boolean): Promise<Response> {
  ...
  const at = now();
  if (!hard) {
    await env.DB.prepare('UPDATE transactions SET deleted_at=?, updated_at=? WHERE id=?').bind(at, at, txId).run();
  ...
  await env.DB.prepare('UPDATE transactions SET refunds_transaction_id=NULL WHERE refunds_transaction_id=?').bind(txId).run();
  await env.DB.prepare('DELETE FROM transaction_tags WHERE transaction_id=?').bind(txId).run();
  await env.DB.prepare('DELETE FROM transaction_splits WHERE transaction_id=?').bind(txId).run();
...
async function restoreTransaction(env: Env, txId: string): Promise<Response> {
  const before = await env.DB.prepare('SELECT * FROM transactions WHERE id=? AND deleted_at IS NOT NULL').bind(txId).first<Row>();
  if (!before) throw new HttpError(404, 'No trashed transaction found');
  const at = now();
  await env.DB.prepare('UPDATE transactions SET deleted_at=NULL, updated_at=? WHERE id=?').bind(at, txId).run();
...
async function purgeAllTrash(env: Env): Promise<{ purged: number }> {
  const trashed = await env.DB.prepare('SELECT id FROM transactions WHERE deleted_at IS NOT NULL').all<Row>();
  for (const t of trashed.results) await deleteTransaction(env, t.id, true);
  return { purged: trashed.results.length };
}

// ---------------- master data ----------------
async function listAll(env: Env, table: string, order: string, includeDeleted = false): Promise<Row[]> {
  const where = includeDeleted ? '' : 'WHERE deleted_at IS NULL ';
  return (await env.DB.prepare(`SELECT * FROM ${table} ${where}ORDER BY ${order}`).all<Row>()).results;
}
...
async function createAccount(env: Env, request: Request): Promise<Response> {
  ...
  const at = now();
  const opening = b['openingBalance'] !== undefined ? Math.round(Number(b['openingBalance']) * 100) : 0;
  const currency = str(b, 'currency') || 'INR';
  const a = { id: id(), name, currency, opening_balance_minor: opening, opening_balance_at: toIso(b['openingBalanceAt'], at), is_active: 1, created_at: at, updated_at: at, deleted_at: null };
  await env.DB.prepare('INSERT INTO accounts (id,name,currency,opening_balance_minor,opening_balance_at,is_active,created_at,updated_at) VALUES (?,?,?,?,?,1,?,?)')
    .bind(a.id, name, currency, opening, a.opening_balance_at, at, at).run();
...
async function updateAccount(env: Env, request: Request, idVal: string): Promise<Response> {
  ...
  if (!name) throw new HttpError(400, 'Account name is required');
  const currency = b['currency'] !== undefined ? String(b['currency']).trim() || 'INR' : before.currency;
  const opening = b['openingBalance'] !== undefined ? Math.round(Number(b['openingBalance']) * 100) : before.opening_balance_minor;
  const updated = { ...before, name, currency, opening_balance_minor: opening, updated_at: at };
  await env.DB.prepare('UPDATE accounts SET name=?,currency=?,opening_balance_minor=?,updated_at=? WHERE id=?')
    .bind(name, currency, opening, at, idVal).run();
...
async function createCategory(env: Env, request: Request): Promise<Response> {
  ...
  const kind = ['expense', 'income', 'both'].includes(String(b['kind'])) ? String(b['kind']) : 'expense';
  if (parentId && !(await exists(env, 'categories', parentId))) throw new HttpError(400, 'Parent category not found');
  const at = now();
  const c = { id: id(), name, parent_id: parentId, kind, sort_order: 0, is_active: 1, created_at: at, updated_at: at, deleted_at: null };
  await env.DB.prepare('INSERT INTO categories (id,name,parent_id,kind,sort_order,is_active,created_at,updated_at) VALUES (?,?,?,?,0,1,?,?)')
    .bind(c.id, name, parentId, kind, at, at).run();
...
async function updateCategory(env: Env, request: Request, idVal: string): Promise<Response> {
  ...
  const at = now();
  const name = b['name'] !== undefined ? String(b['name']).trim() : before.name;
  const parentId = b['parentId'] === null || b['parentId'] === '' ? null : b['parentId'] !== undefined ? String(b['parentId']) : before.parent_id;
  const kind = b['kind'] !== undefined ? (['expense', 'income', 'both'].includes(String(b['kind'])) ? String(b['kind']) : before.kind) : before.kind;
  if (!name) throw new HttpError(400, 'Category name is required');
  ...
  await env.DB.prepare('UPDATE categories SET name=?,parent_id=?,kind=?,updated_at=? WHERE id=?')
    .bind(name, parentId, kind, at, idVal).run();
...
async function createMethod(env: Env, request: Request): Promise<Response> {
  ...
  if (!(await exists(env, 'accounts', accountId))) throw new HttpError(400, 'Account not found');
  const at = now();
  const m = { id: id(), account_id: accountId, name, is_active: 1, created_at: at, updated_at: at, deleted_at: null };
  await env.DB.prepare('INSERT INTO payment_methods (id,account_id,name,is_active,created_at,updated_at) VALUES (?,?,?,?,1,?)')
    .bind(m.id, accountId, name, at, at).run();
...
async function updateMethod(env: Env, request: Request, idVal: string): Promise<Response> {
  ...
  await env.DB.prepare('UPDATE payment_methods SET name=?,account_id=?,updated_at=? WHERE id=?')
    .bind(name, accountId, at, idVal).run();
...
async function createPayee(env: Env, request: Request): Promise<Response> {
  ...
  const at = now();
  const p = { id: id(), name, address, is_active: 1, created_at: at, updated_at: at, deleted_at: null };
  await env.DB.prepare('INSERT INTO payees (id,name,address,is_active,created_at,updated_at) VALUES (?,?,?,1,?,?)')
    .bind(p.id, name, address, at, at).run();
...
async function updatePayee(env: Env, request: Request, idVal: string): Promise<Response> {
  ...
  await env.DB.prepare('UPDATE payees SET name=?,address=?,updated_at=? WHERE id=?').bind(name, address, at, idVal).run();
...
async function createTag(env: Env, request: Request): Promise<Response> {
  ...
  await env.DB.prepare('INSERT INTO tags (id,name,created_at,updated_at) VALUES (?,?,?,?)').bind(t.id, name, at, at).run();
...
async function updateTag(env: Env, request: Request, idVal: string): Promise<Response> {
  ...
  await env.DB.prepare('UPDATE tags SET name=?,updated_at=? WHERE id=?').bind(name, at, idVal).run();
  await audit(env, 'tag', idVal, 'update', before, updated);
  return json(updated);
}

async function softDelete(env: Env, table: string, entityType: string, idVal: string): Promise<Response> {
  const before = await getEntity(env, table, idVal);
  if (!before) throw new HttpError(404, 'Not found');
  const at = now();
  await env.DB.prepare(`UPDATE ${table} SET deleted_at=?, updated_at=? WHERE id=?`).bind(at, at, idVal).run();
...
async function listNotes(env: Env, url: URL): Promise<Response> {
  ...
  const r = await env.DB.prepare(`SELECT * FROM notes n WHERE ${clauses.join(' AND ')} ORDER BY COALESCE(n.reminder_at, n.created_at) DESC LIMIT 500`)
    .bind(...params)
    .all<Row>();
...
async function createNote(env: Env, request: Request): Promise<Response> {
  ...
  if (transactionId && !(await exists(env, 'transactions', transactionId))) throw new HttpError(400, 'Transaction not found');
  const title = str(b, 'title');
  const content = str(b, 'content');
  const reminderAt = optStr(b, 'reminderAt') ? toIso(b['reminderAt']) : null;
  const n = { id: id(), transaction_id: transactionId, title, content, reminder_at: reminderAt, is_done: 0, created_at: at, updated_at: at, deleted_at: null };
  await env.DB.prepare('INSERT INTO notes (id,transaction_id,title,content,reminder_at,is_done,created_at,updated_at) VALUES (?,?,?,?,?,0,?,?)')
    .bind(n.id, n.transaction_id, title, content, reminderAt, at, at).run();
...
async function updateNote(env: Env, request: Request, idVal: string): Promise<Response> {
  ...
  const reminderAt = b['reminderAt'] === null ? null : b['reminderAt'] !== undefined ? toIso(b['reminderAt']) : before.reminder_at;
  const isDone = b['isDone'] !== undefined ? (b['isDone'] ? 1 : 0) : before.is_done;
  const updated = { ...before, title, content, reminder_at: reminderAt, is_done: isDone, updated_at: at };
  await env.DB.prepare('UPDATE notes SET title=?,content=?,reminder_at=?,is_done=?,updated_at=? WHERE id=?')
    .bind(title, content, reminderAt, isDone, at, idVal).run();
...
async function listAttachments(env: Env, url: URL): Promise<Response> {
  ...
  const r = await env.DB.prepare(`SELECT * FROM attachments WHERE ${clauses.join(' AND ')} ORDER BY created_at LIMIT 200`)
    .bind(...params)
    .all<Row>();
...
async function createAttachment(env: Env, request: Request): Promise<Response> {
  ...
  if (!transactionId || !(await exists(env, 'transactions', transactionId))) throw new HttpError(400, 'Valid transactionId is required');
  ...
  const sizeBytes = b['sizeBytes'] !== undefined && b['sizeBytes'] !== null ? asIntSafe(b['sizeBytes']) : null;
  const a = { id: id(), transaction_id: transactionId, provider: kind, external_file_id: url, file_name: fileName, mime_type: mimeType, size_bytes: sizeBytes, created_at: at, updated_at: at, deleted_at: null };
  await env.DB.prepare('INSERT INTO attachments (id,transaction_id,provider,external_file_id,file_name,mime_type,size_bytes,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?)')
    .bind(a.id, a.transaction_id, a.provider, a.external_file_id, fileName, mimeType, sizeBytes, at, at).run();
...
async function listRecurring(env: Env): Promise<Response> {
  const r = await env.DB.prepare(`SELECT * FROM recurring_rules WHERE deleted_at IS NULL ORDER BY is_active DESC, next_due_at`).all<Row>();
  return json(r.results);
}

async function ruleFromBody(env: Env, b: Record<string, unknown>, at: string, defaults: Row): Promise<Record<string, unknown>> {
  const name = str(b, 'name');
  ...
  const interval = b['interval'] !== undefined ? clampInt(b['interval'], 1, 365, defaults.interval_value || 1) : defaults.interval_value || 1;
  ...
  if (!(await exists(env, 'accounts', accountId))) throw new HttpError(400, 'Account not found');
  const categoryId = b['categoryId'] === null ? null : b['categoryId'] !== undefined ? String(b['categoryId']) : defaults.category_id;
  const methodId = b['methodId'] === null ? null : b['methodId'] !== undefined ? String(b['methodId']) : defaults.payment_method_id;
  if (categoryId && !(await exists(env, 'categories', categoryId))) throw new HttpError(400, 'Category not found');
  if (methodId && !(await exists(env, 'payment_methods', methodId))) throw new HttpError(400, 'Payment method not found');
  let payeeId = b['payeeId'] === null ? null : b['payeeId'] !== undefined ? String(b['payeeId']) : defaults.payee_id;
  if (b['payee'] !== undefined) payeeId = await upsertPayee(env, str(b, 'payee'), at);
  const noOfPayments = b['noOfPayments'] === null || b['noOfPayments'] === undefined ? defaults.no_of_payments ?? null : clampInt(b['noOfPayments'], 1, 100000, 1);
  const nextDueAt = b['nextDueAt'] !== undefined ? toIso(b['nextDueAt']) : defaults.next_due_at || at;
  return {
    name, transaction_type: type, account_id: accountId, payment_method_id: methodId, category_id: categoryId,
    payee_id: payeeId, amount_minor: amount, description: b['description'] !== undefined ? String(b['description']) : defaults.description || '',
    note: b['note'] !== undefined ? String(b['note']) : defaults.note || '', frequency, interval_value: interval,
    no_of_payments: noOfPayments, next_due_at: nextDueAt,
  };
}
...
async function createRecurring(env: Env, request: Request): Promise<Response> {
  ...
  await env.DB.prepare(
    ...
    .bind(idVal, f.name, f.transaction_type, f.account_id, f.payment_method_id, f.category_id, f.payee_id, f.amount_minor, f.description, f.note, f.frequency, f.interval_value, f.no_of_payments, f.next_due_at, at, at)
    .run();
  const created = await env.DB.prepare('SELECT * FROM recurring_rules WHERE id=?').bind(idVal).first<Row>();
...
async function updateRecurring(env: Env, request: Request, idVal: string): Promise<Response> {
  ...
  await env.DB.prepare(
    ...
    .bind(f.name, f.transaction_type, f.account_id, f.payment_method_id, f.category_id, f.payee_id, f.amount_minor, f.description, f.note, f.frequency, f.interval_value, f.no_of_payments, f.next_due_at, at, idVal)
    .run();
...
async function generateOne(env: Env, idVal: string): Promise<Response> {
  const rule = await env.DB.prepare('SELECT * FROM recurring_rules WHERE id=? AND deleted_at IS NULL').bind(idVal).first<Row>();
  if (!rule) throw new HttpError(404, 'Recurring rule not found');
  const at = now();
  const count = (await env.DB.prepare('SELECT COUNT(*) AS n FROM transactions WHERE recurring_rule_id=? AND deleted_at IS NULL').bind(idVal).first<Row>())?.n ?? 0;
  if (rule.no_of_payments && count + 1 > rule.no_of_payments) throw new HttpError(400, 'Recurring rule has reached its payment limit');
  const txId = id();
  await env.DB.prepare(INSERT_TX)
    .bind(txId, rule.account_id, rule.payment_method_id, rule.category_id, rule.payee_id, rule.transaction_type, rule.amount_minor, rule.next_due_at, rule.description || rule.name, rule.note || '', 'cleared', '', 0, null, '', null, null, rule.id, 0, at, at)
    .run();
  ...
  await env.DB.prepare('UPDATE recurring_rules SET next_due_at=?, last_generated_at=?, updated_at=? WHERE id=?')
    .bind(next, rule.next_due_at, at, idVal).run();
  await audit(env, 'transaction', txId, 'create', null, { recurring_rule_id: idVal, occurred_at: rule.next_due_at }, { recurring: true });
  const updated = await env.DB.prepare('SELECT * FROM recurring_rules WHERE id=?').bind(idVal).first<Row>();
...
async function skipRecurring(env: Env, idVal: string): Promise<Response> {
  ...
  const at = now();
  const next = advanceDue(rule.next_due_at, rule.frequency, rule.interval_value);
  await env.DB.prepare('UPDATE recurring_rules SET next_due_at=?, updated_at=? WHERE id=?').bind(next, at, idVal).run();
  await audit(env, 'recurring_rule', idVal, 'update', rule, { ...rule, next_due_at: next }, { skipped: true });
...
async function toggleRecurring(env: Env, idVal: string): Promise<Response> {
  ...
  await env.DB.prepare('UPDATE recurring_rules SET is_active=?, updated_at=? WHERE id=?').bind(active, at, idVal).run();
...
async function handleImportCsv(env: Env, request: Request, url: URL): Promise<Response> {
  ...
  let mode: 'append' | 'replace' = url.searchParams.get('mode') === 'replace' ? 'replace' : 'append';
  let csvText = raw;
...
async function handleExportCsv(env: Env, url: URL): Promise<Response> {
  ...
  return textBody(csv, 'text/csv; charset=utf-8', { headers: { 'content-disposition': 'attachment; filename="expenses.csv"' } });
}
...
async function handleBootstrap(env: Env): Promise<Response> {
  const [accounts, categories, methods, payees, tags, suggestions, recurring] = await Promise.all([
    env.DB.prepare(`SELECT * FROM accounts WHERE deleted_at IS NULL AND is_active=1 ORDER BY name`).all<Row>(),
    env.DB.prepare(`SELECT * FROM categories WHERE deleted_at IS NULL AND is_active=1 ORDER BY parent_id IS NOT NULL, sort_order, name`).all<Row>(),
    env.DB.prepare(`SELECT * FROM payment_methods WHERE deleted_at IS NULL AND is_active=1 ORDER BY account_id, name`).all<Row>(),
    env.DB.prepare(`SELECT * FROM payees WHERE deleted_at IS NULL AND is_active=1 ORDER BY name`).all<Row>(),
    env.DB.prepare(`SELECT * FROM tags WHERE deleted_at IS NULL ORDER BY name`).all<Row>(),
    env.DB.prepare(`SELECT description FROM description_suggestions ORDER BY usage_count DESC, last_used_at DESC LIMIT 100`).all<Row>(),
    env.DB.prepare(`SELECT * FROM recurring_rules WHERE deleted_at IS NULL AND is_active=1 ORDER BY next_due_at`).all<Row>(),
  ]);
  return json({
...
async function handleSearchOptions(env: Env, url: URL): Promise<Response> {
  const q = (url.searchParams.get('q') || '').trim();
  if (!q) return json({ descriptions: [], categories: [], methods: [], accounts: [], payees: [], tags: [] });
  const s = `%${q}%`;
  const [descriptions, categories, methods, accounts, payees, tags] = await Promise.all([
    env.DB.prepare('SELECT description AS value, usage_count AS count FROM description_suggestions WHERE description LIKE ? ORDER BY usage_count DESC, last_used_at DESC LIMIT 8').bind(s).all<Row>(),
    env.DB.prepare('SELECT id, name, kind, parent_id FROM categories WHERE deleted_at IS NULL AND is_active=1 AND name LIKE ? ORDER BY name LIMIT 8').bind(s).all<Row>(),
    env.DB.prepare('SELECT id, name, account_id FROM payment_methods WHERE deleted_at IS NULL AND is_active=1 AND name LIKE ? ORDER BY name LIMIT 8').bind(s).all<Row>(),
    env.DB.prepare('SELECT id, name FROM accounts WHERE deleted_at IS NULL AND is_active=1 AND name LIKE ? ORDER BY name LIMIT 8').bind(s).all<Row>(),
    env.DB.prepare('SELECT id, name FROM payees WHERE deleted_at IS NULL AND is_active=1 AND name LIKE ? ORDER BY name LIMIT 8').bind(s).all<Row>(),
    env.DB.prepare('SELECT id, name FROM tags WHERE deleted_at IS NULL AND name LIKE ? ORDER BY name LIMIT 8').bind(s).all<Row>(),
  ]);
  return json({
...
async function handleAudit(env: Env, url: URL): Promise<Response> {
  ...
  const r = await env.DB.prepare(`SELECT * FROM audit_log${where} ORDER BY occurred_at DESC LIMIT 300`).bind(...params).all<Row>();
  return json(r.results);
}
...
export async function route(request: Request, url: URL, env: Env): Promise<Response> {
  ...
  if (seg[1] === 'transactions' && seg[2]) {
    const txId = seg[2];
    if (m === 'GET' && seg[3] === 'audit') return res(await handleAudit(env, new URL(`http://x/api/audit?entityType=transaction&entityId=${encodeURIComponent(txId)}`, url.origin)));
    if (m === 'GET' && !seg[3]) {
    ...
    if (m === 'DELETE' && !seg[3]) return res(await deleteTransaction(env, txId, url.searchParams.get('hard') === '1'));
  ...
  if (p === '/api/trash') {
    if (m === 'GET') return res(await listTransactions(env, new URL(`http://x/api/transactions?deleted=1&limit=500`, url.origin)));
  ...
  const master: Record<string, { table: string; entity: string; order: string }> = {
    ...
    categories: { table: 'categories', entity: 'category', order: 'parent_id IS NOT NULL, sort_order, name' },
    'payment-methods': { table: 'payment_methods', entity: 'payment_method', order: 'account_id, name' },
  ...
  if (seg[1] && master[seg[1]]) {
    ...
    if (m === 'DELETE' && seg[2]) return res(await softDelete(env, spec.table, spec.entity, seg[2]));
  }
  ...
  if (seg[1] === 'notes') {
    ...
    if (seg[2]) {
      if ((m === 'PUT' || m === 'PATCH') && !seg[3]) return res(await updateNote(env, request, seg[2]));
  ...
  if (seg[1] === 'attachments') {
    ...
    if (m === 'DELETE' && seg[2]) return res(await softDelete(env, 'attachments', 'attachment', seg[2]));
  }
  ...
  if (seg[1] === 'recurring') {
    ...
    if (seg[2]) {
      ...
      if (m === 'DELETE' && !seg[3]) return res(await softDelete(env, 'recurring_rules', 'recurring_rule', seg[2]));
  ...
  if (m === 'POST' && (p === '/api/import/backup' || p === '/api/backup/restore')) return res(await handleRestoreBackup(env, request));
  if (m === 'GET' && p === '/api/export/csv') return res(await handleExportCsv(env, url));
  if (m === 'GET' && (p === '/api/export/json' || p === '/api/backup')) return res(json(await exportJson(env)));
  if (m === 'POST' && p === '/api/drive/sync') return res(await handleDriveSync(env));
  if (m === 'GET' && p === '/api/drive/status') return res(json({ configured: driveConfigured(env) }));
